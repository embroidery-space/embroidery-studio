...
- Run the `/codebase-design` skill for the architecture vocabulary and its principles.
...
If `CONTEXT.md` defines "Order," talk about "the Order intake module" --- not "the FooBarHandler," and not "the Order service."
...
Side effects happen inline as decisions crystallize --- run the `/embroiderly-context` skill to keep the domain model current as you go:

- **Naming a deepened module after a concept not in `CONTEXT.md`?**
...
- **Want to explore alternative interfaces for the deepened module?**
  Run the `/codebase-design` skill and use its design-it-twice parallel sub-agent pattern.
